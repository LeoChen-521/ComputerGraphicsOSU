...
const char *WINDOWTITLE = { "OpenGL / Project 3 -- Xiaoru Chen" };
...
bool 	Distort; 			// true means to distort
...
void
DoTextureDistortingmenu( int id )
{
	Distort = id;
	glutSetWindow( MainWindow );
	glutPostRedisplay( );
...
InitMenus( )
{
	...
	int TextureDistortingmenu = glutCreateMenu( DoTextureDistortingmenu );
	...
	glutAddSubMenu(   "Distort",          TextureDistortingmenu);
...
OsuSphere( float radius, int slices, int stacks )
{
	...
	for( int ilat = 0; ilat < NumLats; ilat++ )
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			if (Distort)
			{
				//p->s = ( lng + M_PI    ) / ( 2.* M_PI ) - 0.2 * sin( 360 * Time * 0.1 + lat );
				p->s = ( lng + M_PI    ) / ( 2.* M_PI ) + 0.4 * cos(lat + Time * 100.);
				p->t = ( lat + M_PI/2. ) / M_PI;
			}
			else
			{
...
// Second OsuSphere
// void
// OsuSphere2( float radius, int slices, int stacks )
// {
// 	// set the globals:
...
// 			//If Distort, do something wrong to compute the S and T coordinates

// 			//use the depth to calculate s value on a sin wave

// 			//float newlng = sin( 2. * (lat + 180. * Time*0.2)) * 0.1;

// 			//float newlng = (sin(Time*0.2))*0.1;

// 			//p->s = ( newlng + M_PI    ) / ( 2.*M_PI );
// 			//p->t = ( lat + M_PI/2. ) / M_PI;

// 			//p->s = ( newlng + M_PI    ) / ( 2.*M_PI );

// 			//p->s = (lng + M_PI    ) / ( 2.* M_PI ) + sin( 2. * (lat + 180. * Time*0.2)) * 0.1;
// 			p->s = (lng + M_PI    ) / ( 2.* M_PI ) + sin( 2. * (180. * Time*0.2 + lat)) * 0.1;

// 			p->t = ( lat + M_PI/2. ) / M_PI;

// 			// p->s = ( 2.*lng + M_PI    ) / ( 2.*M_PI );
// 			// p->t = ( lat + M_PI/2. ) / M_PI;
// 		}
// 	}
