...
const char *WINDOWTITLE = { "OpenGL / Project 2 -- Xiaoru Chen" };
...
Animate( )
{
	...
	//float stepAngle = 360. * Time * 0.1;
 	//BladeAngle += stepAngle;

 	BladeAngle = 360. * Time * 5;
...
Display( )
{
	...
	//rotate the Rear blade 2 times faster than the Top blade
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char *)"Project 2 - Animate a Helicopter!" );
...
InitLists( )
{
	...
	glBegin (GL_POLYGON);
		for (int i = 0; i<100; i++)
		{
			glVertex3f (0.5*cos(ang1), 0., 0.5*sin(ang1));
			ang1 += dang1;
		}
	glEnd();
	...
	// A green circle
	glColor3f( 0., 1., 0. );
	float dang2 = 2. *M_PI/(float)(100);
	float ang2 = 0.;

	glBegin (GL_POLYGON);
		for (int i = 0; i<100; i++)
		{ 
			glVertex3f (0.5*cos(ang2), 1., 0.5*sin(ang2));
			ang2 += dang2;
		}
	glEnd();
	...
	glColor3f( 1., 0., 0. );
	float dang3 = 2. *M_PI/(float)(100);
	float ang3 = 0.;
	glBegin (GL_QUADS);
		for (int i = 0; i<25; i++)
		{ 
			glVertex3f (0.5*cos(ang3), 1., 0.5*sin(ang3));
			glVertex3f (0.5*cos(ang3), 0., 0.5*sin(ang3));
	...
	glColor3f( 1., 0., 1. );
	float dang4 = 2. *M_PI/(float)(100);
	float ang4 = ang3;
	glBegin (GL_QUADS);
		for (int i = 0; i<25; i++)
		{ 
			glVertex3f (0.5*cos(ang4), 1., 0.5*sin(ang4));
			glVertex3f (0.5*cos(ang4), 0., 0.5*sin(ang4));
	...
	glColor3f( 1., 1., 0. );
	float dang5 = 2. *M_PI/(float)(100);
	float ang5 = ang4;
	glBegin (GL_QUADS);
		for (int i = 0; i<25; i++)
		{
			glVertex3f (0.5*cos(ang5), 1., 0.5*sin(ang5));
			glVertex3f (0.5*cos(ang5), 0., 0.5*sin(ang5));
	...
	glColor3f( 1., 1., 1. );
	float dang6 = 2. *M_PI/(float)(100);
	float ang6 = ang5;
	glBegin (GL_QUADS);
		for (int i = 0; i<25; i++)
		{
			glVertex3f (0.5*cos(ang6), 1., 0.5*sin(ang6));
			glVertex3f (0.5*cos(ang6), 0., 0.5*sin(ang6));
	...
	// create the display list for the heli
