...
const char *WINDOWTITLE = { "OpenGL / Project 5 -- Xiaoru Chen" };
...
// Global variables for project 5
GLuint  SphereList;
...
// 	VertTime = (float)ms  /  (float)MS_IN_THE_ANIMATION_CYCLE;   
...
// 	FragTime = (float)ms  /  (float)MS_IN_THE_ANIMATION_CYCLE;   
...
Display( )
{
    ...
    if (BothShaders) 
    {
    	VertTime = FragTime = Time;
    	uSize = 1.5 * sin(5. * M_PI * FragTime); // Time equation on uSize
    }
	if (VertShaderOnly)
	{
		VertTime = Time;
	...
	Pattern->SetUniformVariable( "uTime",  VertTime );
 ...
 //    Pattern->Use(0); // go back to fixed-function OpenGL
	...
	//Pattern->SetUniformVariable( "uVertTime",  VertTime );
	//Pattern->SetUniformVariable( "uFragTime",  FragTime );
